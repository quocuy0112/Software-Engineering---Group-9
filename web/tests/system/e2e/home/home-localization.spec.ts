...
test("locale switching covers visible and assistive Home copy without changing records or search state", async ({
  page,
}) => {
  await page.goto("/");
  ...
  await expect(
    page.getByRole("heading", {
      name: "Tìm việc theo năng lực. CV được chấm điểm minh bạch.",
    }),
  ).toBeVisible();
  await expect(page.getByLabel("Mở menu điều hướng")).toHaveAttribute(
    "aria-expanded",
    "false",
  );
  ...
  await expect(
    page.getByText("Illustrative Smart Match example"),
  ).toBeVisible();
});
