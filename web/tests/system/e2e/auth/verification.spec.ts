...
test("shows generic invalid/reused outcomes and supports safe resend", async ({
  page,
}) => {
  ...
  await expect(
    page.getByText(/The link is invalid, expired, already used/i),
  ).toBeVisible();

  await page.getByLabel("Email address").fill(email);
  const resend = page.waitForResponse(
    (response) =>
      response.url().endsWith("/api/identity/verification/resend") &&
