...
import {
  failRegistrationRequest,
  failResendRequest,
} from "../fixtures/failures";

test("renders recoverable registration and email-provider failures without false auth", async ({
  page,
}) => {
  ...
  await page
    ...
    .fill(`failure-${Date.now()}@example.test`);
  ...
  const response = page.waitForResponse(
    (result) =>
      result.url().endsWith("/api/identity/register") &&
      result.request().method() === "POST",
  );
  await page.getByRole("button", { name: "Create account" }).click();
  expect((await response).status()).toBe(503);
  await expect(
    page.getByRole("alert").filter({ hasText: /could not|try again/i }),
  ).toBeVisible();
  expect(
