...
describe("StartConversation", () => {
  it("opens an eligible context with keyboard-accessible controls", async () => {
    ...
    expect(screen.getByRole("dialog", { name: /start a conversation/i })).toBeVisible();
    fireEvent.click(screen.getByRole("button", { name: /message recruiter b/i }));
    await waitFor(() => expect(onOpened).toHaveBeenCalledWith("conversation-1"));
  });

  it("renders a meaningful empty state", () => {
    render(<StartConversation csrfProof="csrf" initialItems={[]} onOpened={() => undefined} />);
    expect(screen.getByText(/no eligible people/i)).toBeVisible();
  });
});
