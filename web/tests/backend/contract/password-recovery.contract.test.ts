...
describe("password recovery HTTP contract", () => {
  it("normalizes request email and requires a strict matching reset object", () => {
    expect(
      forgotPasswordSchema.parse({ email: " User@Example.Test " }).email,
    ...
    expect(
      resetPasswordSchema.safeParse({
        token: "opaque",
        newPassword: "correct horse 2026",
        confirmPassword: "different value 2026",
      }).success,
    ).toBe(false);
  });
  ...
  it("returns a clear invalid-email error while keeping reset-token failures secret-free and no-store", async () => {
    const requested = await forgot(
      post("/api/identity/password/forgot", { email: "not-an-email" }),
    );
  ...
  it("documents the fail-closed saga, 2FA preservation, and never models returned credentials", async () => {
    ...
    expect(openapi).toContain(
      "'503': {$ref: '#/components/responses/ResetIncomplete'}",
    );
    expect(openapi).toContain(
      "login stays blocked until mandatory cleanup and finalization complete",
    );
