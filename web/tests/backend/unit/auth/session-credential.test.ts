...
} from "@/backend/auth/policy/cookie-policy";
...
describe("exclusive browser-session credential", () => {
  it("uses unprefixed insecure local cookie names", () => {
    ...
    expect(policy).toEqual({
      sessionName: "smarthire.session",
      preAuthName: "smarthire.pre-auth",
      attributes: { httpOnly: true, sameSite: "lax", secure: false, path: "/" },
    });
  });
  ...
  it("uses exact secure production cookie names without a Domain", () => {
    ...
    const options = {
      session: { expiresIn: 604740 },
      advanced: betterAuthCookieOptions(policy),
    };
