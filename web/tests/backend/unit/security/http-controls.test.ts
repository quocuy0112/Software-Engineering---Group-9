...
import { validateCsrfRequest } from "@/backend/security/csrf/csrf";
...
describe("identity HTTP controls", () => {
  it("uses exactly one authentication cookie and one non-authenticating challenge cookie per environment", () => {
    ...
    expect(local).toMatchObject({
      ...
      preAuth: { name: "smarthire.pre-auth", attributes: { secure: false } },
    });
    ...
    expect(clearCookieAttributes(production.session.attributes)).toMatchObject({
      ...production.session.attributes,
      maxAge: 0,
    });
    expect(
      [production.session.name].filter((name) => name.includes("session")),
    ).toHaveLength(1);
  });
  it("requires exact origin, same-origin fetch metadata, and CSRF proof", () => {
    const request = (origin: string, site: string, proof: string) =>
      new Request("https://app.example.test/api/state", {
        method: "POST",
        headers: { origin, "sec-fetch-site": site, "x-csrf-token": proof },
      });
    expect(
      validateCsrfRequest(
        request("https://app.example.test", "same-origin", "proof"),
    ...
    expect(
      validateCsrfRequest(
        request("https://evil.test", "same-origin", "proof"),
        "https://app.example.test",
        "proof",
      ),
    ).toBe(false);
    expect(
      validateCsrfRequest(
        request("https://app.example.test", "cross-site", "proof"),
        "https://app.example.test",
        "proof",
      ),
    ).toBe(false);
    expect(
      validateCsrfRequest(
        request("https://app.example.test", "same-origin", "wrong"),
        "https://app.example.test",
        "proof",
      ),
    ).toBe(false);
  ...
  it.each([
    ...
    expect(
      trustedInternalRedirect(value, "https://app.example.test", "/home"),
    ).toBe("/home");
  });
  it("accepts an exact-origin relative redirect", () =>
    expect(
      trustedInternalRedirect(
