...
describe("Feature 009 Group 2 verification boundaries", () => {
  it("shares transaction-local locks and checks between decisions", () => {
    const eligibility = source("src/backend/admin/verification/verification-decision-eligibility.ts");
    const approval = source("src/backend/admin/verification/verification-approval-transaction.ts");
    const review = source("src/backend/admin/verification/verification-review-service.ts");
  ...
  it("persists one dual-channel outcome and exposes only Approve/Reject", () => {
    const notification = source("src/backend/admin/notifications/verification-notification-event.ts");
    const panel = source("src/frontend/features/admin/verification/verification-decision-panel.tsx");
