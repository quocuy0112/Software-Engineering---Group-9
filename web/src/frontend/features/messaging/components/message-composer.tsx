  ...
  return (
    <section className="messaging-composer" aria-label="Message composer">
      <ul className="messaging-outbox" aria-label="Outgoing messages">
        {outbox.map((item) => (
          <li key={item.clientOperationId} data-status={item.status.toLocaleLowerCase()}>
            <span className="messaging-outbox-copy">{item.content}</span>
            <span className="messaging-outbox-status">
              {item.status === "PENDING" ? <span className="messaging-spinner" aria-hidden="true" /> : null}
      ...
      >
        ...
        <textarea
          aria-label="Message"
          placeholder={disabled ? "Messaging is blocked" : "Write a message..."}
          value={content}
          maxLength={2_000}
          disabled={disabled}
        ...
        <button className="messaging-send-button" type="submit" disabled={disabled}>
          <span>Send</span>
      ...
      <p className="messaging-composer-hint">Press Enter to send · Shift + Enter for a new line</p>
      {validation ? <p className="messaging-inline-alert" role="alert">{validation}</p> : null}
    </section>
  );
}
