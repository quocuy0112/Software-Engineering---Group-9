...
import type { PrivateMatchResponse, PrivateMatchStatus } from "@/shared/contracts/private-cv-match";
import {
...
import { PrivateMatchAnalysisSteps, PrivateMatchStepper } from "./private-match-shared";

function LoadingScreen({ data }: { data?: PrivateMatchStatus }) {
  const job = data?.job;
  return (
    <main className="private-match-page" aria-busy="true" aria-live="polite">
      <div className="private-match-breadcrumb">CV Match Check <span>/</span> Analysis</div>
      <div className="private-match-title-row">
        <div>
          <h1>Analyzing your CV</h1>
          <p>SmartHire is comparing the selected CV with the job requirements.</p>
        </div>
        <span className="private-match-badge private-match-badge--blue">
          <LoaderCircle className="private-match-spin" aria-hidden="true" /> Analysis in progress
        </span>
      </div>
      ...
      <section className="private-match-processing-banner">
        ...
        <div>
          ...
          <strong>You can leave this page — we&apos;ll save the result when it&apos;s ready.</strong>
        </div>
      </section>
      <div className="private-match-columns">
        <div className="private-match-main-column">
          <section className="private-match-card">
            ...
            <p className="private-match-section-intro">Each stage completes safely before the report is published.</p>
            <PrivateMatchAnalysisSteps activeStep={data?.state === "ANALYZING" ? 2 : 1} />
          </section>
        </div>
        <aside className="private-match-sidebar">
          <section className="private-match-card">
            <div className="private-match-card-label">SELECTED JOB</div>
            <h2>{job?.title ?? "Your selected role"}</h2>
            <p>{job ? `${job.company} · ${job.location}` : "The selected job"}</p>
            <span className="private-match-private-line">
              <ShieldCheck aria-hidden="true" /> Private preview · Not shared with the employer
            </span>
          </section>
          <section className="private-match-card">
            <h2>What happens next</h2>
            <p>When the analysis is complete, you can review the preview before choosing whether to apply.</p>
          </section>
        </aside>
      </div>
    </main>
    ...
    <main className="private-match-page private-match-state-page">
      ...
      <section className="private-match-limit-card" role="alert">
        ...
        <div>
          ...
          <p>The report was not published because the source evidence could not be analyzed safely.</p>
          <div className="private-match-state-actions">
            <Link className="private-match-primary-button" href={retryHref}>
              <RefreshCw aria-hidden="true" /> Try again
            </Link>
            <Link className="private-match-secondary-button" href="/cv-match-check">
...
function UnavailableScreen() {
  return (
    <main className="private-match-page private-match-state-page">
      <div className="private-match-breadcrumb">CV Match Check <span>/</span> Report</div>
  ...
  return (
    <main className="private-match-page private-match-state-page">
      ...
      <section className="private-match-card private-match-empty" role="alert">
        ...
        <button className="private-match-primary-button" type="button" onClick={onRetry}>
          <RefreshCw aria-hidden="true" /> Try again
        </button>
      </section>
    </main>
