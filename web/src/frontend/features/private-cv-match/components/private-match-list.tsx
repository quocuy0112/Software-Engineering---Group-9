...
import { BriefcaseBusiness, FileText, Plus, RefreshCw, ShieldCheck, TriangleAlert } from "lucide-react";
...
function ListSkeleton() {
  return (
    <div className="private-match-list" aria-busy="true" aria-live="polite">
      {[1, 2, 3].map((item) => (
        <div className="private-match-list-row private-match-list-row--skeleton" key={item}>
          <span />
          <div><span /><span /></div>
          <span />
        </div>
      ))}
      <span className="private-match-visually-hidden">Loading saved CV match checks</span>
    </div>
  );
}

function ListItem({ item, onDeleted }: { item: PrivateMatchListItem; onDeleted: () => void }) {
  ...
  return (
    <article className="private-match-list-row">
      ...
      <Link className="private-match-list-main" href={`/cv-match-check/${encodeURIComponent(item.checkId)}`}>
        <strong>{item.job.title}</strong>
        <span>{item.job.company} · {item.job.location}</span>
        <small><FileText aria-hidden="true" /> {item.cv.fileName} · v{item.cv.version}</small>
      </Link>
      <div className="private-match-list-score">
        ...
        <small>Created {dateFormatter.format(new Date(item.createdAt))} · Expires in {daysUntil(item.expiresAt)} days</small>
      </div>
      <PrivateMatchDeleteControl checkId={item.checkId} compact onDeleted={onDeleted} />
    </article>
  );
}
...
export function PrivateMatchList() {
  ...
  const items = query.data?.items.filter((item) => !removedIds.has(item.checkId)) ?? [];
  return (
    <main className="private-match-page">
      ...
      <div className="private-match-title-row private-match-list-title-row">
        ...
        <Link className="private-match-primary-button" href="/cv-match-check/new">
      ...
      <section className="private-match-card private-match-list-intro">
        ...
        <div>
          ...
          <p>Only you can see these reports. They never change a recruiter&apos;s ranking or your application.</p>
        </div>
      </section>

      {query.isPending ? <ListSkeleton /> : query.isError ? (
        <section className="private-match-limit-card" role="alert">
          ...
          <div>
            ...
            <button className="private-match-secondary-button" type="button" onClick={() => void query.refetch()}>
        ...
        <section className="private-match-card private-match-empty private-match-list-empty">
          ...
          <p>Get a private, explainable preview for one job before you apply.</p>
          <Link className="private-match-primary-button" href="/cv-match-check/new">
        ...
        <section aria-labelledby="saved-checks-heading">
          ...
          <div className="private-match-list">
            {items.map((item) => <ListItem item={item} key={item.checkId} onDeleted={() => setRemovedIds((current) => new Set(current).add(item.checkId))} />)}
          </div>
        </section>
      )}
    </main>
...
function GaugeIcon() {
  return <span className="private-match-list-empty-icon" aria-hidden="true">%</span>;
}
