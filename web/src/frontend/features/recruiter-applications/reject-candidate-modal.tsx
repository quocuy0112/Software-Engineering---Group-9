...
export function RejectCandidateModal({
  ...
  const reject = async () => {
    ...
    try {
      ...
      if (!response.ok)
        throw new Error(
          payload?.message ?? "The candidate could not be rejected.",
        );
      onCompleted();
    } catch (cause) {
      setError(
  ...
  return (
    <RankingModalFrame
      ...
      subtitle={`${candidate.candidate.displayName} · Final human decision`}
      icon="×"
      info={
        <>
          <AlertTriangle aria-hidden="true" />
          <span>
            AI did not make this decision. Your reason, identity, and timestamp
            will be stored in stage history.
          </span>
        </>
      }
    ...
    >
      ...
      <label className="ai-ranking-field">
        <span>
          Internal note{" "}
          <small>
            Optional · internal use only · never sent to the candidate
          </small>
        </span>
        <textarea
          value={internalNote}
