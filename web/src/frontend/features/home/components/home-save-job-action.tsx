...
export function HomeSaveJobAction({
  jobId,
  slug,
  initialSaved,
  csrfProof,
  ...
  if (!csrfProof)
    return (
      <HomeAuthRequiredFeedback
        returnTo={`/jobs/${slug}`}
        label={copy.common.accountRequired}
      />
    );
  ...
  async function toggle() {
    ...
    try {
      ...
      setFeedback({
        role: "status",
        message: before ? copy.jobs.removeSuccess : copy.jobs.saveSuccess,
      });
    } catch {
      setSaved(before);
  ...
  return (
    <span className="home-save-action">
      <button
        ...
        onClick={() => void toggle()}
      >
        <HomeSaveIcon saved={saved} />
      </button>
      {feedback ? (
        <span className="home-action-feedback" role={feedback.role}>
          {feedback.message}
        </span>
      ) : null}
    </span>
