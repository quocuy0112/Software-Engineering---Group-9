import { adminJson, adminRouteError, commandHeaders, parseAdminJson, AdminHttpError } from "@/backend/admin/http/admin-route";
...
export async function POST(request: Request, context: { params: Promise<{ jobId: string; action: string }> }) {
  try {
    const authority = await new AdminRequestBoundary().require(request, { sensitive: true });
    ...
    if (command.command !== expected) throw new AdminHttpError(400, "VALIDATION_FAILED");
    const headers = commandHeaders(request, { strictIfMatch: true });
    if (!headers.idempotencyKey || !Number.isInteger(headers.expectedVersion)) throw new AdminHttpError(400, "VALIDATION_FAILED");
    return adminJson(await new JobPostManagementService().command(authority, jobId, command, headers.expectedVersion, headers.idempotencyKey));
