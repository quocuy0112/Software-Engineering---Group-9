...
export async function POST(request: Request, context: { params: Promise<{ applicationId: string }> }) {
  ...
  let raw: unknown;
  try { raw = await request.json(); } catch { return NextResponse.json({ code: "INVALID_REQUEST", message: "Confirm the interview transition." }, { status: 400, headers: noStore }); }
  try {
    const result = await new RecruiterApplicationDecisionService().moveToInterview({ userId: current.userId, sessionId: current.sessionId, applicationId: (await context.params).applicationId, idempotencyKey: request.headers.get("idempotency-key") ?? "", raw });
    ...
    return NextResponse.json({ code: unavailable ? "UNAVAILABLE" : conflict ? "CONFLICT" : "INVALID_REQUEST", message: unavailable ? "The application is not available." : conflict ? "This decision is no longer valid. Refresh the application." : "Confirm the transition and provide a valid expected stage version." }, { status: unavailable ? 404 : conflict ? 409 : 400, headers: noStore });
  }
}
