...
import { decisionOutcomeSchema, rejectDecisionRequestSchema } from "@/shared/contracts/scoring";
...
export async function POST(request: Request, context: { params: Promise<{ applicationId: string }> }) {
  try {
    ...
    const raw = await parseBoundedJson(request, rejectDecisionRequestSchema, 8 * 1024);
    const result = await new RecruiterApplicationDecisionService().reject({ ...current, applicationId: (await context.params).applicationId, idempotencyKey, raw });
    ...
    return accountJson({ code: "VALIDATION_ERROR", message: "Choose a valid rejection reason and confirm the decision." }, { status: 400 });
  }
}
