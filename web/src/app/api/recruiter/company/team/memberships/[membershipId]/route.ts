import { accountErrorResponse, accountJson, parseBoundedJson, requireAccountRequest } from "@/backend/security/account-request-boundary";
...
export async function PATCH(request: Request, context: { params: Promise<{ membershipId: string }> }) {
  try {
    ...
    const command = await parseBoundedJson(request, teamMembershipCommandSchema, 1024);
    await new CompanyTeamService().change(actor.userId, (await context.params).membershipId, command.action, command.role);
    ...
    if (error instanceof CompanyTeamCommandError) return accountJson({ code: error.code, message: "This member cannot be updated." }, { status: 409 });
