...
> - `UC-SCR-01: Execute Hybrid Candidate Screening` belongs to the scope of **Diagram 5 (Supporting Services and Analytics)**. It is included by `UC-SCR-03` and is not re-specified here; refer to the Diagram 5 document for its detailed flow and corresponding screenshots (`UI_01_*`).
...
> **Note:** `UC-SCR-01` appears in the tables below only as an included/referenced use case (owned by Diagram 5) and is therefore not listed in the scope table above.
