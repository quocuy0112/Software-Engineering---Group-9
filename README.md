...
Feature 014 business-registry lookup is enabled in the environment template with `BUSINESS_REGISTRY_PROVIDER=vietqr` and uses VietQR's public business endpoint; set it to `disabled` to force the supported manual fallback. `BUSINESS_REGISTRY_TIMEOUT_MS` and `BUSINESS_REGISTRY_RESPONSE_LIMIT_BYTES` bound server-side requests and responses. A matched or partial result confirms that the tax identifier has a registry record but never auto-approves an employer.
