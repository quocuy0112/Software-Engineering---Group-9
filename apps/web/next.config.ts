...
const nextConfig: NextConfig = {
  ...
  async headers() {
    ...
    const noStore = [{ key: "Cache-Control", value: "no-store, max-age=0" }, { key: "Pragma", value: "no-cache" }];
    return [
      { source: "/:path*", headers: globalHeaders },
      ...["/verify-email", "/settings/:path*", "/two-factor/:path*", "/reset-password/:path*"].map((source) => ({ source, headers: noStore })),
    ];
  },
};
