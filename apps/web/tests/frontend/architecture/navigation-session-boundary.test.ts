...
describe("navigation session boundary", () => {
  ...
  it("keeps child pages free of duplicated shell markup and session fetches", async () => {
    ...
    for (const path of pages) {
      ...
      expect(source).not.toMatch(/requireSession|WorkspaceShell|localStorage|sessionStorage/);
    }
  });

  it("keeps workspace navigation presentation-only and non-persistent", async () => {
    const shell = await readFile(
      "src/frontend/features/authentication/components/auth/workspace-shell.tsx",
      "utf8",
    );
    const navigation = await readFile(
      "src/frontend/features/authentication/components/auth/workspace-navigation.tsx",
      "utf8",
    );
