...
async function pending(
  ...
  const created = await registration.create({
    ...
    tokenDigest: protector.digest(raw),
    protectedToken: protector.seal(raw),
    expiresAt,
...
describe("email verification", () => {
  it("activates once and rejects reuse", async () => {
    ...
    expect(
      await prisma.userAccount.findUnique({
        ...
        select: { state: true, emailVerified: true },
      }),
    ).toEqual({ state: "ACTIVE", emailVerified: true });
  });
  it("rejects invalid and expired values", async () => {
    expect(
      (await new VerifyEmailService().execute(protector.generate())).success,
    ).toBe(false);
    const expiresAt = new Date(Date.now() + 1000);
    const fixture = await pending("expired", expiresAt);
    expect(
      await new VerifyEmailService().execute(
        fixture.raw,
  ...
  it("allows exactly one concurrent verification", async () => {
    ...
    const results = await Promise.all([
      new VerifyEmailService().execute(fixture.raw),
      new VerifyEmailService().execute(fixture.raw),
    ]);
    expect(results.filter((result) => result.success)).toHaveLength(1);
    expect(
      await prisma.securityToken.count({
        where: { userId: fixture.userId, status: "CONSUMED" },
      }),
    ).toBe(1);
  });
  it.each(["ACTIVE", "SUSPENDED", "DELETED"] as const)(
    "rejects a token for a %s account",
    async (state) => {
      ...
      expect(
        (await new VerifyEmailService().execute(fixture.raw)).success,
      ).toBe(false);
    },
  );
  it("replaces active tokens on resend and is enumeration resistant", async () => {
    ...
    expect(
      await prisma.securityToken.count({
        where: { userId: fixture.userId, status: "ACTIVE" },
      }),
    ).toBe(1);
    expect(
      await prisma.securityToken.count({
        where: { userId: fixture.userId, status: "SUPERSEDED" },
      }),
    ).toBe(1);
  });
  it("rate limits resend generically", async () => {
    ...
    for (let index = 0; index < 4; index++)
      results.push(await service.execute(email, subject));
  ...
  it("writes a retrievable local capture without logging the token", async () => {
    ...
    expect(
      await deliverOutboxMessage(fixture.outboxId, new CaptureEmailAdapter()),
    ).toBe(true);
    const after = (await readdir(directory)).filter(
  ...
  it("dead-letters repeated delivery failure and emits a secret-free audit event", async () => {
    ...
    const failing = {
      send: vi.fn().mockRejectedValue(new Error("provider secret details")),
    };
    for (let attempt = 0; attempt < 5; attempt++)
      expect(await deliverOutboxMessage(fixture.outboxId, failing)).toBe(false);
    const row = await prisma.emailOutbox.findUniqueOrThrow({
      where: { id: fixture.outboxId },
    });
  ...
  it("keeps retryable SMTP failures retryable and dead-letters terminal SMTP failures immediately", async () => {
    ...
    expect(
      await deliverOutboxMessage(retryable.outboxId, temporaryAdapter),
    ).toBe(false);
    expect(
    ...
    expect(await deliverOutboxMessage(terminal.outboxId, terminalAdapter)).toBe(
      false,
    );
    expect(
      await prisma.emailOutbox.findUniqueOrThrow({
        where: { id: terminal.outboxId },
        select: { status: true, attempts: true, safeErrorCode: true },
      }),
    ).toEqual({
      status: "DEAD",
      attempts: 1,
      safeErrorCode: "SMTP_AUTH_FAILED",
    });
  });
});
