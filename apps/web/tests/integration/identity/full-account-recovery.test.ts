...
describe("full account recovery controlled-clock integration", () => {
  it("is enumeration-safe, rate-limited, and queues only eligible verified 2FA accounts", async () => {
    ...
    expect(unknown).toMatchObject({ accepted: true, status: 202 });
    expect(known.message).toBe(unknown.message);
    expect(
      await prisma.emailOutbox.count({
        where: { userId: fixture.userId, templateVersion: "account-recovery-confirmation.v1" },
      }),
    ).toBe(1);
  ...
  it("starts one exact 24-hour hold, revokes access, blocks login, and leaves 2FA unchanged", async () => {
    ...
    expect(operation.holdEndsAt.getTime() - operation.holdStartedAt.getTime()).toBe(
      24 * 60 * 60 * 1000,
    );
    expect(await prisma.session.count({ where: { userId: fixture.userId } })).toBe(0);
    expect(
      await prisma.authenticationChallenge.count({ where: { userId: fixture.userId } }),
    ).toBe(0);
    expect(
    ...
    expect(
      await prisma.twoFactor.findUniqueOrThrow({ where: { userId: fixture.userId } }),
    ...
    const blocked = await new LoginWithPasswordService().execute(
      ...
      { headers: noSessionHeaders(), subject: "recovery-test", now: new Date(operation.holdStartedAt.getTime() + 1000) },
    );
  ...
  it("cancels exactly once and preserves existing password and 2FA", async () => {
    ...
    const confirmation = await issueConfirmation(fixture.email, now);
    const operation = await confirm(confirmation, new Date(now.getTime() + 1));
    const cancellationProof = protector.unseal(operation.cancellationProofCiphertext);
    const results = await Promise.all([
      new CancelFullAccountRecoveryService().execute(cancellationProof, new Date(operation.holdStartedAt.getTime() + 1000)),
      new CancelFullAccountRecoveryService().execute(cancellationProof, new Date(operation.holdStartedAt.getTime() + 1000)),
    ]);
    expect(results.filter((result) => result.ok)).toHaveLength(1);
    expect(results.filter((result) => !result.ok)).toHaveLength(1);
    const cancelled = await prisma.fullAccountRecoveryOperation.findUniqueOrThrow({
      where: { id: operation.id },
    });
    expect(cancelled.status).toBe("CANCELLED");
    expect(
      await prisma.emailOutbox.count({
        where: { idempotencyKey: cancelled.cancellationNotificationIdempotencyKey },
      }),
    ).toBe(1);
    expect(
      await prisma.userAccount.findUniqueOrThrow({
  ...
  it("rejects completion before the hold and completes once after it, disabling old factors only then", async () => {
    ...
    const confirmation = await issueConfirmation(fixture.email, now);
    const operation = await confirm(confirmation, new Date(now.getTime() + 1));
    const completionProof = protector.unseal(operation.completionProofCiphertext);
    const beforeHold = await new CompleteFullAccountRecoveryService().execute(
      completionProof,
      replacementPassword,
      new Date(operation.holdEndsAt.getTime() - 1),
    );
    expect(beforeHold).toMatchObject({ ok: false, holdEndsAt: operation.holdEndsAt });
    ...
    expect(await prisma.session.count({ where: { userId: fixture.userId } })).toBe(0);
    expect(await prisma.twoFactor.findUnique({ where: { userId: fixture.userId } })).toBeNull();
    expect(
      await prisma.userAccount.findUniqueOrThrow({
        where: { id: fixture.userId },
        select: { twoFactorEnabled: true },
  ...
  it("keeps raw proofs out of operation, outbox, audit, and failure records", async () => {
    ...
    const cancellation = protector.unseal(operation.cancellationProofCiphertext);
    const evidence = JSON.stringify({
      operation,
      outbox: await prisma.emailOutbox.findMany({ where: { userId: fixture.userId } }),
      audits: await prisma.auditEvent.findMany({
        where: { targetType: "account_recovery", targetId: operation.id },
      }),
    });
    for (const secret of [confirmation, completion, cancellation, fixture.secret, ...fixture.backupCodes]) {
