...
const execute = vi.fn();
vi.mock("@/server/services/identity/reset-password", () => ({
...
describe("reset-password route", () => {
  ...
  it("returns generic invalid errors and rejects cross-origin requests", async () => {
    expect((await POST(request({ token: "x", newPassword: "short", confirmPassword: "short" }))).status).toBe(400);
    expect((await POST(request({ token: "x", newPassword: "correct horse 2026", confirmPassword: "correct horse 2026" }, false))).status).toBe(403);
