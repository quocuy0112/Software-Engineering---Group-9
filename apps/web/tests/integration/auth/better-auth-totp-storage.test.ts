...
async function request(path: string, options: { body?: unknown; cookie?: string } = {}) {
...
async function enroll() {
  ...
  const enable = await request("/two-factor/enable", { cookie, body: { password } });
...
describe.sequential("Better Auth 1.6.11 TOTP storage protection (PostgreSQL)", () => {
  it("bootstraps an ACTIVE account", async () => {
    const response = await request("/sign-up/email", { body: { name: "Storage User", email, password } });
  ...
  it("persists the TOTP secret encrypted at rest, not in plaintext", async () => {
    ...
    const decrypted = await symmetricDecrypt({ key: serverEnvironment.BETTER_AUTH_SECRET, data: stored.secret });
  ...
  it("persists backup codes encrypted at rest, never as readable code text", async () => {
    ...
    const decrypted = await symmetricDecrypt({ key: serverEnvironment.BETTER_AUTH_SECRET, data: stored.backupCodes });
  ...
  it("fails safely when the stored ciphertext is tampered with", async () => {
    ...
    await expect(symmetricDecrypt({ key: serverEnvironment.BETTER_AUTH_SECRET, data: tampered })).rejects.toBeTruthy();
    // A ciphertext produced under a different key must not decrypt with the server key.
    const foreign = await symmetricEncrypt({ key: "a-different-secret-key-2026", data: "spoofed" });
    await expect(symmetricDecrypt({ key: serverEnvironment.BETTER_AUTH_SECRET, data: foreign })).rejects.toBeTruthy();
  });

  it("rotates persisted material, replacing prior data on re-enrollment", async () => {
    const before = await prisma.twoFactor.findFirstOrThrow({ where: { user: { normalizedEmail: email } } });
    const { totpURI } = await enroll();
    const rows = await prisma.twoFactor.findMany({ where: { user: { normalizedEmail: email } } });
    ...
    const decrypted = await symmetricDecrypt({ key: serverEnvironment.BETTER_AUTH_SECRET, data: after.secret });
  ...
  it("remains the exclusive persistence owner: no SmartHire secret table exists", async () => {
    ...
    const twoFactorTables = await prisma.$queryRaw<Array<{ table_name: string }>>`
      SELECT table_name FROM information_schema.tables
