...
const message = { kind: "VERIFY_EMAIL" as const, recipient: "existing-adapter@example.test", subject: "Verify", html: "<p>Verify</p>", text: "Verify", idempotencyKey: "verification:existing-adapter" };
...
describe("existing email adapters", () => {
  ...
  it("keeps Resend delivery functional through its provider boundary", async () => {
    ...
    const adapter = new ResendEmailAdapter(() => ({ emails: { send } } as never), { apiKey: "re_test", from: "SmartHire <sender@example.test>" });
    await expect(adapter.send(message)).resolves.toEqual({ providerMessageId: "resend-message-id" });
    expect(send).toHaveBeenCalledWith(expect.objectContaining({ to: message.recipient, headers: { "Idempotency-Key": message.idempotencyKey } }));
  });
});
