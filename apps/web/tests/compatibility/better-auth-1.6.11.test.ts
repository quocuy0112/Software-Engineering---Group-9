...
async function request(path: string, options: { body?: unknown; cookie?: string; method?: string } = {}) {
  const headers = new Headers({ origin: "http://localhost:3000" });
  if (options.body !== undefined) headers.set("content-type", "application/json");
  if (options.cookie) headers.set("cookie", options.cookie);
  return auth.handler(new Request(`${baseURL}${path}`, { method: options.method ?? (options.body === undefined ? "GET" : "POST"), headers, body: options.body === undefined ? undefined : JSON.stringify(options.body) }));
}
...
describe.sequential("Better Auth 1.6.11 PostgreSQL compatibility", () => {
  it("creates Better Auth credential ownership with a non-plaintext password hash", async () => {
    const response = await request("/sign-up/email", { body: { name: "Compatibility User", email, password } });
    expect(response.status, await response.clone().text()).toBe(200);
    const user = await prisma.userAccount.findUniqueOrThrow({ where: { normalizedEmail: email }, include: { accounts: true, sessions: true } });
  ...
  it("rejects Pending Verification sessions through SmartHire state enforcement", async () => {
    ...
    expect(await prisma.session.count({ where: { user: { normalizedEmail: email } } })).toBe(0);
    await prisma.userAccount.update({ where: { normalizedEmail: email }, data: { state: "ACTIVE" } });
  });

  it("verifies passwords, creates one cookie-backed database session, lists and revokes sessions", async () => {
    const wrong = await request("/sign-in/email", { body: { email, password: "incorrect password" } });
    expect(wrong.status).toBe(401);
    const first = await signIn();
    expect(first.status).toBe(200);
    const firstCookie = responseCookie(first, "smarthire.session");
    expect(firstCookie).toBeTruthy();
    expect(first.headers.getSetCookie().filter((value) => value.startsWith("smarthire.session="))).toHaveLength(1);
    expect(first.headers.getSetCookie().some((value) => /jwt/i.test(value))).toBe(false);
    ...
    const firstToken = (await prisma.session.findFirstOrThrow({ where: { user: { normalizedEmail: email } }, orderBy: { createdAt: "desc" } })).token;
    expect(firstCookie).toContain(firstToken);

    await signIn();
    const secondToken = (await prisma.session.findFirstOrThrow({ where: { user: { normalizedEmail: email }, token: { not: firstToken } }, orderBy: { createdAt: "desc" } })).token;
    const listed = await request("/list-sessions", { cookie: firstCookie! });
    expect(listed.status).toBe(200);
    expect((await listed.json()) as unknown[]).toHaveLength(2);

    const revokeOne = await request("/revoke-session", { cookie: firstCookie!, body: { token: secondToken } });
    expect(revokeOne.status).toBe(200);
    expect(await prisma.session.findUnique({ where: { token: secondToken } })).toBeNull();

    const signOut = await request("/sign-out", { cookie: firstCookie!, body: {} });
    expect(signOut.status).toBe(200);
    expect(await prisma.session.findUnique({ where: { token: firstToken } })).toBeNull();
    ...
    const revokeAll = await request("/revoke-sessions", { cookie: thirdCookie, body: {} });
  ...
  it("persists encrypted TOTP and backup codes, challenges sign-in, and consumes one backup code atomically", async () => {
    ...
    const enable = await request("/two-factor/enable", { cookie: sessionCookie, body: { password } });
    ...
    const stored = await prisma.twoFactor.findFirstOrThrow({ where: { user: { normalizedEmail: email } } });
    ...
    const verifyEnrollment = await request("/two-factor/verify-totp", { cookie: sessionCookie, body: { code } });
    expect(verifyEnrollment.status, await verifyEnrollment.clone().text()).toBe(200);
    expect((await prisma.twoFactor.findUniqueOrThrow({ where: { userId: stored.userId } })).verified).toBe(true);
    const enrollmentCookie = responseCookie(verifyEnrollment, "smarthire.session") ?? sessionCookie;
    await request("/sign-out", { cookie: enrollmentCookie, body: {} });
    expect(await prisma.session.count({ where: { userId: stored.userId } })).toBe(0);

    const challengeA = await signIn();
    const challengeB = await signIn();
    expect(challengeA.status).toBe(200);
    expect((await challengeA.json() as { twoFactorRedirect: boolean }).twoFactorRedirect).toBe(true);
    ...
    const challengeSessionHeaders = challengeA.headers.getSetCookie().filter((value) => value.startsWith("smarthire.session="));
    expect(challengeSessionHeaders.at(-1)).toMatch(/Max-Age=0/);
    expect(await prisma.session.count({ where: { userId: stored.userId } })).toBe(0);

    const attempts = await Promise.all([
      request("/two-factor/verify-backup-code", { cookie: preAuthA, body: { code: enrollment.backupCodes[0] } }),
      request("/two-factor/verify-backup-code", { cookie: preAuthB, body: { code: enrollment.backupCodes[0] } }),
    ]);
    ...
    const authenticatedCookie = responseCookie(authenticated, "smarthire.session")!;
    authoritativeCookie = authenticatedCookie;

    const regenerate = await request("/two-factor/generate-backup-codes", { cookie: authenticatedCookie, body: { password } });
    ...
    const storedReplacement = await prisma.twoFactor.findUniqueOrThrow({ where: { userId: stored.userId } });
    expect(storedReplacement.backupCodes).not.toBe(stored.backupCodes);
    expect(storedReplacement.backupCodes).not.toContain(replacement.backupCodes[0]);
    const oldCodeChallenge = await signIn();
    const oldCodeCookie = responseCookie(oldCodeChallenge, "smarthire.pre-auth")!;
    const oldCodeAttempt = await request("/two-factor/verify-backup-code", { cookie: oldCodeCookie, body: { code: enrollment.backupCodes[1] } });
  ...
  it("rejects Suspended and Deleted sessions and has no email OTP implementation", async () => {
    ...
    await expect(getActiveSession(new Headers({ cookie: authoritativeCookie }))).resolves.toBeNull();
    await prisma.userAccount.update({ where: { normalizedEmail: email }, data: { state: "ACTIVE", twoFactorEnabled: false } });
    await prisma.twoFactor.deleteMany({ where: { user: { normalizedEmail: email } } });
    ...
    await prisma.userAccount.update({ where: { normalizedEmail: email }, data: { state: "DELETED", deletedAt: new Date() } });
    await expect(getActiveSession(new Headers({ cookie: activeCookie }))).resolves.toBeNull();
