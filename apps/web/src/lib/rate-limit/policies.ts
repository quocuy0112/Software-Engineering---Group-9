...
export const rateLimitPolicies = {
  ...
  verificationResend: { scope: "verification-resend", limit: 3, windowSeconds: 60 * 60 },
  ...
  totpEnrollment: { scope: "totp-enrollment", limit: 5, windowSeconds: 10 * 60 },
} as const;

export function safeRetryMetadata(decision: RateLimitDecision) {
  return { retryAfterSeconds: Math.max(1, decision.retryAfterSeconds), message: "Please wait before trying again." };
}
