...
export class PrismaPreAuthRepository {
  ...
  async create(userId: string, browserBinding: string, now = new Date()) {
    ...
    await prisma.authenticationChallenge.create({
      data: {
        ...
        expiresAt: new Date(now.getTime() + PRE_AUTH_LIFETIME_MS),
        maxAttempts: PRE_AUTH_MAX_ATTEMPTS,
        createdAt: now,
      },
    });
  ...
  async claimAttempt(
    ...
    const digest = this.protector.digest(token),
      context = this.protector.digest(`browser:${browserBinding}`);
  ...
  async releaseFailed(id: string, claimTime: Date) {
    await prisma.authenticationChallenge.updateMany({
      where: { id, consumedAt: claimTime },
      data: { consumedAt: null },
    });
  }
  async finalize(id: string, userId: string, claimTime: Date, step?: bigint) {
    return prisma.$transaction(async (tx) => {
      if (step !== undefined) {
        const replay = await tx.authenticationChallenge.findFirst({
          where: { userId, verifiedTotpStep: step, consumedAt: { not: null } },
          select: { id: true },
        });
        if (replay) return false;
      }
      const result = await tx.authenticationChallenge.updateMany({
        where: { id, userId, consumedAt: claimTime },
        data: { verifiedTotpStep: step ?? null },
      });
