...
export const backupCodeSchema = z.object({ code: z.string().trim().min(8).max(128) }).strict();
export type BackupCode = z.infer<typeof backupCodeSchema>;

export const completeTwoFactorSchema = z.discriminatedUnion("factor", [
  z.object({ factor: z.literal("totp"), code: z.string().regex(/^\d{6}$/, "Enter the 6-digit code.") }).strict(),
  z.object({ factor: z.literal("backup-code"), code: z.string().trim().min(8).max(128) }).strict(),
]);
export type CompleteTwoFactor = z.infer<typeof completeTwoFactorSchema>;
export const TWO_FACTOR_GENERIC_ERROR = "Verification could not be completed. Sign in and try again.";
