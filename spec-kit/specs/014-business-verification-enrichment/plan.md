...
Admission uses the existing PostgreSQL `RateLimitBucket`: 10 account lookups per 15 minutes and 30 identifier lookups per 15 minutes, plus 5 challenge sends per account/email binding per hour. Subject values are HMAC-digested before rate-limit persistence. Successful matched snapshots may be reused by the same applicant for 15 minutes; every returned preparation reference is an opaque server ID and responses are private/no-store.
...
2. `POST registry-lookups` validates exactly ten ASCII digits, applies rate limits, calls or bypasses the configured gateway, creates an immutable 30-minute snapshot, replaces the current preparation binding, and supersedes prior email challenges.
...
4. `POST company-email/challenges` normalizes the email, binds a random 256-bit token digest to applicant + current snapshot + tax identifier + email, supersedes earlier active challenges, and transactionally queues one `COMPANY_EMAIL_VERIFY` outbox item. The email link places the raw token in the URL fragment so it is not sent in HTTP referrers or initial server requests.
5. The Candidate page removes the fragment with `history.replaceState` and posts the token to `company-email/confirm`. The service atomically verifies the digest, applicant, current binding, status, and expiry.
6. Final multipart submission revalidates every field, challenge age, snapshot expiry, current preparation version, evidence, company match, and prerequisite. It stores evidence through the existing adapter and then transactionally creates request facts, request, evidence metadata, one receipt, challenge consumption, and snapshot acceptance. Storage is deleted if the transaction fails.
