...
- [x] T001 Add `test:job-board` and `perf:job-board` scripts in `web/package.json` and root `package.json`
- [x] T002 [P] Add Feature 003 test-path and privacy-safe fixture conventions in `web/tests/helpers/job-board-database-fixture.ts`
- [x] T003 [P] Add the job-board stylesheet import boundary in `web/src/app/globals.css` and create `web/src/frontend/features/jobs/styles/job-board.css`
...
- [x] T009 [P] Write failing schema/unknown-field/OpenAPI fixture tests in `web/tests/shared/unit/contracts/jobs/job-contracts.test.ts`
- [x] T010 [P] Write failing Vietnamese normalization and cursor tests in `web/tests/backend/unit/jobs/search-normalization.test.ts`
- [x] T011 Implement bounded Vietnamese normalization and versioned opaque cursors in `web/src/backend/services/jobs/search-normalization.ts`
- [x] T012 Implement the protected job request boundary using the existing active session, same-origin, and CSRF controls in `web/src/backend/security/job-request-boundary.ts`
- [x] T013 [P] Add shared job service errors, public field projection types, and action-state types in `web/src/backend/services/jobs/job-types.ts`
- [x] T014 Complete isolated PostgreSQL company/job/question/CV/candidate fixture lifecycle in `web/tests/helpers/job-board-database-fixture.ts`
...
- [x] T015 [P] [US1] Write failing query validation, availability, relevance, tie-break, and cursor unit tests in `web/tests/backend/unit/jobs/job-search-policy.test.ts`
- [x] T016 [P] [US1] Write failing PostgreSQL visibility/filter/sort/pagination/concurrency tests in `web/tests/backend/integration/jobs/job-search.test.ts`
- [x] T017 [P] [US1] Write failing GET `/api/jobs` contract/public-field tests in `web/tests/backend/contract/jobs/job-search.contract.test.ts`
...
- [x] T019 [US1] Implement parameterized indexed public search/count and keyset ordering in `web/src/backend/repositories/jobs/prisma-public-job-repository.ts`
- [x] T020 [US1] Implement validation, normalization, public availability, projection, and cursor orchestration in `web/src/backend/services/jobs/job-discovery-service.ts`
- [x] T021 [US1] Implement the thin public search handler in `web/src/app/api/jobs/route.ts`
...
- [x] T024 [P] [US2] Write failing public-detail availability/action projection unit tests in `web/tests/backend/unit/jobs/job-detail-policy.test.ts`
- [x] T025 [P] [US2] Write failing GET `/api/jobs/{slug}` neutral-response and field-allowlist tests in `web/tests/backend/contract/jobs/job-detail.contract.test.ts`
...
- [x] T027 [US2] Add public detail and actor-scoped action-state queries in `web/src/backend/repositories/jobs/prisma-public-job-repository.ts`
- [x] T028 [US2] Add active/closed/expired projection, neutral unavailability, and cache policy in `web/src/backend/services/jobs/job-discovery-service.ts`
- [x] T029 [US2] Implement the thin detail handler whose `[jobId]` route segment carries the public slug in `web/src/app/api/jobs/[jobId]/route.ts`
...
- [x] T031 [US2] Implement metadata/canonical URL, not-found/error behavior, and server-rendered detail page in `web/src/app/jobs/[slug]/page.tsx`
...
- [x] T032 [P] [US3] Write failing application eligibility, answer validation, consent, and snapshot tests in `web/tests/backend/unit/jobs/application-policy.test.ts`
- [x] T033 [P] [US3] Write failing transaction rollback, uniqueness race, idempotency binding, audit, and notification atomicity tests in `web/tests/backend/integration/jobs/job-application.test.ts`
- [x] T034 [P] [US3] Write failing application-form/submission contract and authorization tests in `web/tests/backend/contract/jobs/job-application.contract.test.ts`
- [x] T035 [P] [US3] Write failing application form, CV/question/consent, error-focus, retry, and accessible pending-state tests in `web/tests/frontend/components/jobs/job-application.test.tsx` and `web/tests/frontend/accessibility/jobs/job-application.accessibility.test.tsx`

### Implementation for User Story 3

- [x] T036 [P] [US3] Implement required-profile, confirmed-CV, question/answer, consent, and bounded snapshot policy in `web/src/backend/services/jobs/application-policy.ts`
- [x] T037 [US3] Implement locked/retry-safe form query and transactional application/audit/notification persistence in `web/src/backend/repositories/jobs/prisma-job-application-repository.ts`
- [x] T038 [US3] Implement application form and idempotent submission orchestration in `web/src/backend/services/jobs/job-application-service.ts`
- [x] T039 [P] [US3] Implement the protected application-form handler in `web/src/app/api/jobs/[jobId]/application-form/route.ts`
- [x] T040 [US3] Implement the protected idempotent submission handler in `web/src/app/api/jobs/[jobId]/applications/route.ts`
- [x] T041 [US3] Implement accessible CV selection, questions, cover letter, consent, confirmation, and authoritative error recovery in `web/src/frontend/features/jobs/components/job-application-form.tsx`
- [x] T042 [US3] Wire the Apply action/dialog into `web/src/frontend/features/jobs/components/job-detail.tsx`
...
- [x] T043 [P] [US4] Write failing ownership, idempotency, race, failure, and CSRF tests in `web/tests/backend/integration/jobs/saved-job.test.ts`
- [x] T044 [P] [US4] Write failing saved-action pending/success/error/reconciliation accessibility tests in `web/tests/frontend/components/jobs/save-job-action.test.tsx`

### Implementation for User Story 4

- [x] T045 [US4] Implement composite-key create-or-read/delete persistence in `web/src/backend/repositories/jobs/prisma-saved-job-repository.ts`
- [x] T046 [US4] Implement target validation and authoritative saved-state orchestration in `web/src/backend/services/jobs/saved-job-service.ts`
- [x] T047 [US4] Implement protected idempotent PUT/DELETE handler in `web/src/app/api/saved-jobs/[jobId]/route.ts`
- [x] T048 [US4] Implement the accessible reconciled save/remove control in `web/src/frontend/features/jobs/components/save-job-action.tsx` and wire it into list/detail cards
...
- [x] T049 [P] [US5] Write failing reason/detail normalization, duplicate, rate-limit, audit, privacy, and rollback tests in `web/tests/backend/integration/jobs/job-report.test.ts`
- [x] T050 [P] [US5] Write failing report contract, neutral response, and protected-boundary tests in `web/tests/backend/contract/jobs/job-report.contract.test.ts`
- [x] T051 [P] [US5] Write failing accessible dialog, conditional detail, cancel, pending, duplicate, retry, and focus-restoration tests in `web/tests/frontend/components/jobs/report-job-dialog.test.tsx`

### Implementation for User Story 5

- [x] T052 [US5] Implement nullable unresolved-key report persistence and audit transaction in `web/src/backend/repositories/jobs/prisma-job-report-repository.ts`
- [x] T053 [US5] Implement report policy, existing database rate limit, neutral duplicate, and non-enforcement orchestration in `web/src/backend/services/jobs/job-report-service.ts`
- [x] T054 [US5] Implement the protected report handler in `web/src/app/api/jobs/[jobId]/reports/route.ts`
- [x] T055 [US5] Implement and wire the accessible report dialog in `web/src/frontend/features/jobs/components/report-job-dialog.tsx` and `web/src/frontend/features/jobs/components/job-detail.tsx`
...
- [x] T056 [P] Add OpenAPI-to-Zod parity and protected/public cache tests in `web/tests/backend/contract/jobs/job-board-openapi-parity.test.ts`
- [x] T057 [P] Add frontend/server/provider/import boundary tests in `web/tests/architecture/job-board-boundaries.test.ts`
- [x] T058 [P] Add desktop/mobile public browse/detail E2E coverage in `web/tests/system/e2e/job-board/job-discovery.spec.ts`
- [x] T059 [P] Add desktop/mobile login-return/save/report/apply and failure-recovery E2E coverage in `web/tests/system/e2e/job-board/job-actions.spec.ts`
- [x] T060 Implement documented 100-sample p95 search/detail/action harness in `web/scripts/measure-job-board-performance.mjs`
- [x] T061 Update current features, commands, architecture, privacy, and validation documentation in `README.md` and `docs/operations/job-board-data-lifecycle.md`
...
- [x] T071 Record requirement traceability, migration truth, integration summary, release blockers, and usability status in `spec-kit/specs/003-job-board-and-advanced-search/checklists/`
...
- T056-T059, T061, T066, and T069-T070 are separate files and can proceed after the corresponding behavior stabilizes.
