...
- [x] T069 [P] [US2] Write failing authorization/comparison tests for owned review-ready drafts, live Profile loading at read time, Profile-aware initial `ADD`/`REPLACE` decisions with unique owned replacement targets, preservation of saved candidate choices, no live-profile copy in payload, bounded verified evidence, explicit empty/missing provenance without fabricated context, foreign/expired/deleted behavior, and target-child ownership in web/tests/backend/integration/cv-import/cv-draft-comparison.test.ts
...
- [x] T076 [P] [US2] Implement owned draft/current-Profile comparison queries with sanitized bounded evidence, Profile-aware initial `ADD` for absent/unmatched data, `REPLACE` for populated scalars or uniquely matched collection entries, safe `SKIP` for ambiguous matches/existing skills, preservation of saved review decisions, and only target Profile child IDs needed for replace choices in web/src/backend/repositories/cv-import/prisma-cv-draft-query-repository.ts
