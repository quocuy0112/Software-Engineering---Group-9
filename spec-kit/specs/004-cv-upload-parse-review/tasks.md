...
- [x] T077 [US2] Implement transaction-bound complete draft payload compare-and-swap, reviewed Profile revision checks, canonical byte caps, immutable parser provenance separation, and no partial saves in web/src/backend/repositories/cv-import/prisma-cv-draft-command-repository.ts
- [x] T078 [US2] Implement comparison retrieval and save orchestration with Feature 002 validation, full-payload normalization, safe duplicate hints, current Profile comparison, and conflict metadata in web/src/backend/services/cv-import/cv-draft-comparison-service.ts
...
- [x] T083 [US2] Implement review query/save/confirm client state with in-memory edits, stable proposal keys, idempotency reuse, duplicate-submit prevention, server revision reconciliation, and unload warning in web/src/frontend/features/cv-import/client/use-cv-draft-review.ts
