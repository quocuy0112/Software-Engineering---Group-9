...
A candidate chooses whether other candidates can find their profile by exact account ID and selects which professional sections are visible in that result and profile view. The candidate can later withdraw this visibility without changing their application records.
...
2. **Given** a discoverable candidate, **When** they change discovery to hidden, **Then** future candidate searches and profile visits no longer expose their live profile.
3. **Given** a candidate has not enabled discovery, **When** another candidate knows their ID, **Then** the other candidate cannot obtain a visible profile result.
...
- Repeated invalid ID searches, whether from one account or one network origin, are throttled, recorded, and handled without revealing which IDs exist.
...
- **FR-002**: The system MUST allow a candidate to independently choose visibility for avatar, headline, summary, general location, skills, experience, education, and professional links.
...
- **FR-007**: The system MUST enforce discovery throttles per signed-in account and per network origin, temporarily restrict excessive failed lookups, and record unusual lookup patterns for review.
...
- **FR-009**: A candidate who changes a profile or section to hidden MUST have that change take effect for future candidate discovery and future live-profile views without changing submitted application snapshots.
...
- **FR-014**: A recruiter MUST NOT receive an applicant’s live professional section when the candidate has not permitted that section for recruiter review after applying.
...
- **FR-016**: A recruiter MUST receive candidate contact details only when the candidate gave contact consent for that same application.
...
- **Candidate profile visibility**: The candidate’s discoverability status and permitted audience for each professional profile section.
...
- **Application contact consent**: A candidate’s explicit decision about whether authorized recruiters for one application may view application contact details.
...
- **SC-005**: In application-consent testing, 100% of recruiter contact views match the candidate’s recorded consent for that specific application.
...
- Candidate contact sharing is off by default and is requested separately for every submitted application.
