...
- [ ] T022 [P] [US2] Add active/hidden/self/section-projection integration tests in `web/tests/backend/integration/profile-discovery/candidate-lookup.test.ts`.
...
- [ ] T026 [US2] Implement lookup admission, neutralization, projection, and minimal audit writes in `web/src/backend/services/profile/candidate-profile-discovery-service.ts`.
...
- [ ] T034 [P] [US3] Add candidate drawer profile-action/component tests in `web/tests/frontend/scoring/candidate-score-drawer-profile.test.tsx`.
...
- [ ] T048 [P] Add profile-discovery performance measurement with P95 report fields in `web/scripts/measure-profile-discovery-performance.mjs` and `web/tests/performance/profile-discovery/lookup-performance.test.ts`.
...
All 50 tasks use the required checkbox, sequential task ID, optional `[P]`, story label for story work, and exact file paths.
