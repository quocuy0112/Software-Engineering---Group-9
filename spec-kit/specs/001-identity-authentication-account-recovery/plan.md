...
1. Registration normalizes email, relies on a unique database constraint, transactionally creates `UserAccount`, Better Auth `AuthProviderAccount`, base Candidate identity, one-time verification token, audit event, and outbox row, and maps a duplicate normalized email to a field-associated 409 response.
