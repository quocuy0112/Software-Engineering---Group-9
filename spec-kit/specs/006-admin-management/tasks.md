...
- [x] T013 Extend the authoritative Prisma model with PlatformAdministratorGrant, AdministratorSessionPolicy, Company verification/tax fields, versioned CompanyMembership REMOVED state, verification/evidence entities, moderation entities, rationale, command receipt, SecurityNotificationWork including additive `emailOutboxId`/`opsAlertedAt`, verification lifecycle relations to the existing EmailOutbox, and dashboard snapshot in `web/prisma/schema.prisma` (FR-001, FR-020–FR-022, FR-024–FR-058, FR-062)
- [x] T014 Create the additive schema/data migrations, indexes, partial uniqueness rules including verification-event EmailOutbox idempotency, provider-truth linkage in `web/prisma/migrations/021_security_notification_provider_truth/migration.sql`, the idempotent one-shot reconciliation command in `web/scripts/reconcile-security-notifications.mjs`, legacy JobReport category mapping, and reversible verification checks (FR-034, FR-035, FR-037, FR-046, FR-048, FR-058, FR-061)
...
- [x] T018 [P] Implement durable SecurityNotificationWork persistence, business-event idempotency, provider-truth outbox reconciliation, leasing, manual-intervention transition, exactly-once ops-alert CAS, and non-rollback behavior only for FR-022 access notifications in `web/src/backend/admin/notifications/` and `web/src/backend/repositories/admin/prisma-security-notification-repository.ts`; the security worker never owns provider attempts (FR-022, FR-058)
...
- [x] T024 [P] Implement separately leased snapshot, evidence, verification-deadline, notification, and retention loops with isolated readiness; the security-notification loop only creates/links EmailOutbox and reconciles provider truth while the email worker exclusively owns provider attempts in `web/src/backend/admin/workers/admin-worker-runtime.ts`, `web/src/backend/admin/workers/admin-worker-entry.ts`, and `web/src/backend/admin/workers/security-notification-loop.ts` (FR-010, FR-021, FR-022, FR-026–FR-028, FR-058)
...
- [x] T056 [P] [US3] Add end-to-end account search, revoke-one/all, suspend, reinstate, wrong-target prevention, and notification failure workflows in `web/tests/system/e2e/admin-management/account-security.spec.ts` (US3-AS1–AS10, SC-004–SC-006). **Blocked by completed T013/T014/T018/T024/T053/T059/T063/T065/T066 and by a runnable `ADMIN_E2E_READY=1` fixture environment; email checks are necessary but do not replace the original workflow checks.**
  - [x] SUSPENDED email becomes eligible only after the transaction containing all-session revocation commits; content contains state/time/session/support facts and no rationale, fraud signal, or admin identity.
  - [x] REINSTATED returns the account to ACTIVE, requires a new sign-in, and states that independently suspended memberships are not restored.
  - [x] All-session revoke creates exactly one email while single-session revoke creates no SecurityNotificationWork or EmailOutbox.
  - [x] Double-click, step-up timeout, and network retry preserve one adminOperationId and create one audit/work/email business event.
  - [x] Provider success produces EmailOutbox SENT plus SecurityNotificationWork DELIVERED; permanent/exhausted failure produces DEAD plus MANUAL_INTERVENTION_REQUIRED plus one safe ops alert.
  - [x] Original account search, wrong-target, revoke, and stale-conflict workflows pass independently of the email checks.
...
- [x] T065 [US3] Add rationale cleanup plus notification enqueue/link/reconciliation and exactly-once safe ops-alert integration; keep all provider retries and SENT/DEAD transitions exclusively in the email worker in `web/src/backend/admin/workers/rationale-retention-loop.ts`, `web/src/backend/admin/workers/security-notification-loop.ts`, and `web/src/backend/email/workers/email-outbox.ts` (FR-021, FR-022, SC-012, SC-018)
- [x] T066 [US3] Add typed `admin-security-v1` renderer cases for ACCOUNT_SUSPENDED, ACCOUNT_REINSTATED, and ALL_SESSIONS_REVOKED containing only safe resulting state/time/session/support facts in `web/src/backend/admin/notifications/account-security-templates.tsx` and `web/src/backend/admin/notifications/renderer-registry.tsx` (FR-021, FR-022)
...
- [x] T074 [P] [US4] Add end-to-end clean/unsafe submission, review decisions, concurrent approval, applicant cancellation/resubmission, existing-company denial, simulated 15/24/72-hour outages, and applicant email delivery truth in `web/tests/system/e2e/admin-management/employer-verification.spec.ts` (US4-AS1–AS14, SC-007, SC-008, SC-010). **Blocked by completed T071/T075/T079/T082/T083/T089/T090/T091 and by scanner/storage/provider fixtures; email checks do not close the upload, outage, concurrency, or prerequisite scope.**
  - [x] Approval email contains company display-name snapshot, CompanyMembership role, server-rendered Recruiter workspace link, and unchanged Candidate identity statement.
  - [x] Two concurrent approvals and a replay of one stable admin operation create exactly one approval email.
  - [x] All seven `verification-v1` event kinds render and transient/permanent provider results drive SENT/DEAD truth.
  - [x] Asynchronous SAFE/UNSAFE upload, protected storage, resubmit, and exact 15/24/72-hour outage workflows pass.
  - [x] Existing-company prerequisite denial and concurrent decision behavior pass independently of email assertions.
...
- [x] T083 [US4] Implement atomic new-company and existing-company approval transactions including unique tax ID, exact CompanyMembership role, prerequisite consumption, request/history, audit, and exactly one business-event-keyed EmailOutbox row snapshotting companyDisplayName and approvedMembershipRole in `web/src/backend/admin/verification/verification-approval-transaction.ts` (FR-032–FR-035, FR-037, FR-038, FR-058)
...
- [x] T089 [P] [US4] Build VerificationDecisionPanel for request changes, rejection, approval, private note, resubmission boundary, explicit confirmation, step-up, conflict recovery, and one stable adminOperationId across retries in `web/src/frontend/features/admin/verification/verification-decision-panel.tsx` (FR-031–FR-035, FR-059)
- [x] T090 [P] [US4] Implement typed business-event-keyed EmailOutbox payload builders and all seven `verification-v1` renderer cases for receipt/change/approval/rejection/cancellation/delay/expiry, with server-rendered workspace URL and no private signals, notes, locators, or admin identity in `web/src/backend/admin/notifications/verification-templates.tsx`, `web/src/backend/admin/notifications/verification-outbox.ts`, and `web/src/backend/admin/notifications/renderer-registry.tsx` (FR-026, FR-028, FR-037)
...
- [x] T097 [P] [US5] Add end-to-end suspend/restore/remove, no-active-membership denial, Candidate preservation, stale recruiter-operation, and company-scoped email workflows in `web/tests/system/e2e/admin-management/company-membership.spec.ts` (US5-AS1–AS6, SC-009, SC-010). **Blocked by completed T095/T098/T100/T104/T105 and a runnable multi-company E2E fixture; email checks do not close the last-owner, isolation, or stale-operation scope.**
  - [x] Suspend, restore, and remove each create exactly one provider-truth email for the selected transition.
  - [x] Content contains only the company display-name snapshot and membership state and never implies account suspension.
  - [x] Stale/conflicting commands create no additional email.
  - [x] Candidate identity and memberships in other companies remain unchanged.
  - [x] Last-OWNER, no-active-membership, and stale recruiter-operation protections pass independently of email assertions.
...
- [x] T100 [US5] Bind membership state, audit, rationale, business-event-keyed notification work with transition-time companyDisplayName snapshot, command receipt, and denied conflicts as one outcome in `web/src/backend/admin/memberships/admin-membership-command-transaction.ts` (FR-039–FR-045, FR-058)
...
- [x] T104 [P] [US5] Build SuspendMembershipDialog, RestoreMembershipDialog, and stronger RemoveMembershipDialog using shared rationale/step-up/conflict primitives plus one stable adminOperationId across retries in `web/src/frontend/features/admin/memberships/membership-action-dialog.tsx` (FR-039, FR-040, FR-041, FR-045)
- [x] T105 [US5] Add typed `admin-security-v1` membership suspend/restore/remove renderer cases using only the company display-name snapshot and new membership state, plus account-detail delivery projection, in `web/src/backend/admin/notifications/membership-security-templates.tsx` and `web/src/backend/admin/notifications/renderer-registry.tsx` (FR-022, FR-039–FR-041)
...
- [x] T140 [P] Complete SC-002 dashboard/list, two-second session/designation, document qualification, worker-deadline, and email reliability reporting in `web/scripts/measure-admin-management-performance.mjs` and `web/tests/performance/admin-management/release-thresholds.test.ts`; include provider latency distribution, retry count by eventKind, MANUAL_INTERVENTION_REQUIRED rate, and commit-to-SENT duration (SC-002–SC-004, SC-007, SC-015, SC-018). **Closure is blocked until T056, T074, and T097 execute in the target environment.**
  - [x] T056, T074, and T097 executed serially in the controlled environment: 16/16 Playwright cases passed with no skip.
  - [x] The exact SC-002 dataset and 10 independent administrator sessions ran for 15 minutes: 8,922 samples, p95 311.64 ms, 99.989% within two seconds, and 0% errors.
  - [x] Four real transaction-to-revocation measurements all completed within two seconds (p95/max 376.61 ms).
  - [x] Reliability evidence reports provider p50/p95/p99, retries by all 13 event kinds, 2/28 MANUAL_INTERVENTION_REQUIRED outcomes, and mean commit-to-SENT 5,813.46 ms.
  - [x] Focused qualification, deadline, worker, contract, security, architecture, and performance suite passed 152/152 tests.
...
## Phase 11: User Story 8 - SmartHire Support Center (Priority: P1)

### Tests for User Story 8

- [x] T161 [P] [US8] Add shared support contract tests in `web/tests/shared/unit/support/support-contracts.test.ts` (FR-063–FR-074; SC-019–SC-024)
- [x] T162 [P] [US8] Add repository lifecycle, quota, assignment, email, worker, and retention integration tests in `web/tests/backend/integration/support/support-repository.test.ts` (FR-064–FR-073; SC-020–SC-024)
- [x] T163 [P] [US8] Add Candidate/admin Route Handler authorization and concurrency contract tests in `web/tests/backend/contract/support/support-routes.contract.test.ts` (FR-063–FR-074; SC-019–SC-023)
- [x] T164 [P] [US8] Add architecture/privacy tests in `web/tests/architecture/support-boundaries.test.ts` and `web/tests/security/support/support-privacy.test.ts` (FR-066, FR-071–FR-074; SC-019, SC-022, SC-024)
- [x] T165 [P] [US8] Add requester and Support Inbox component/accessibility tests under `web/tests/frontend/components/support/` (FR-063–FR-072; SC-019–SC-023)

### Implementation for User Story 8

- [x] T166 [US8] Add support schema and migration in `web/prisma/schema.prisma` and `web/prisma/migrations/*_support_center/migration.sql` (FR-063–FR-074)
- [x] T167 [US8] Add shared support contracts and safe errors under `web/src/shared/contracts/support/` and `web/src/backend/support/http/` (FR-063–FR-074)
- [x] T168 [US8] Implement support repository transactions and workers under `web/src/backend/repositories/support/` (FR-064–FR-073)
- [x] T169 [US8] Implement Candidate/admin boundaries and support services under `web/src/backend/support/` (FR-063–FR-074)
- [x] T170 [US8] Add Candidate/admin Route Handlers under `web/src/app/api/support/` and `web/src/app/api/admin/support-cases/` (FR-063–FR-074)
- [x] T171 [US8] Add content-free `/support` Socket.IO gateway and client invalidation hook in `web/src/backend/support/realtime/`, shared contracts, and `web/server.ts` (FR-070, FR-072, FR-074)
- [x] T172 [US8] Add content-free support email rendering and outbox dispatch in `web/src/backend/admin/notifications/` and `web/src/backend/email/workers/email-outbox.ts` (FR-066, FR-071)
- [x] T173 [US8] Add auto-close, authority-loss requeue, and retention cycles under `web/src/backend/admin/workers/` (FR-065, FR-068, FR-073)
- [x] T174 [US8] Build responsive Candidate `/support` workspace under `web/src/app/(workspace)/support/` and `web/src/frontend/features/support/` (FR-063–FR-066, FR-070–FR-074)
- [x] T175 [US8] Build and register React Admin `support-cases` resource under `web/src/frontend/features/admin/` (FR-067–FR-070, FR-074)
- [x] T176 [US8] Extend audit allowlists, contracts, scripts, and validation documentation (FR-063–FR-074; SC-019–SC-024)

## Phase 12: Support Center Release Gates

- [x] T177 Run Prisma, typecheck, lint, format, focused Support suites, admin/messaging regressions, and production build; record evidence in `spec-kit/specs/006-admin-management/release-validation.md` (SC-019–SC-024)
- [x] T178 Audit requester ownership, assignee enforcement, one active assignment, content-free outputs, no ordinary-chat reader, bounded workers, exact retention, and exclusion of `.claude/settings.local.json`; mark Support tasks complete (FR-063–FR-074; SC-019–SC-024)
