...
- [x] Requests are enumeration-safe; verified-email confirmation is required;
...
- PostgreSQL migration `005_full_account_recovery` is applied after migration
  ...
  all passed.
- Current focused recovery Vitest: 10 files, 25 tests passed, including
  enumeration/rate limiting, fragment-proof routes, exact hold, cancellation
...
- Focused Playwright recovery: desktop Chromium 1/1 passed and mobile-320 1/1
  passed in separate reproducible production-stack runs. The workflow covers
  enumeration-safe request, verified-email confirmation, lower-assurance
